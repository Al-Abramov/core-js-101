...
function timeSpanToString(startDate, endDate) {
  ...
  const milSec = endDate.getMilliseconds() - startDate.getMilliseconds();
  ...
  return `${addZero(hour)}:${addZero(min)}:${addZero(sec)}.${milSec > 10 ? milSec : '000'}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const min = curDate.getUTCMinutes();

  let deltaAngle = Math.abs(0.5 * (60 * hour - 11 * min)) % 360;
